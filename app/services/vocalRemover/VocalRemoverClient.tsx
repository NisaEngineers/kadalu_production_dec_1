...
export default function VocalRemoverClient() {
  ...
  const handleUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files?.[0]) {
      ...
      setUserId(generateUserId()); // ALWAYS generate new ID
    }
  };
  ...
  const handleRemoveVocals = async () => {
    ...
    try {
      ...
      data.stems.forEach((s: any) => {
        const mp3Url = s.mp3 ? `${baseUrl}${s.mp3}` : "";
        const wavUrl = s.wav ? `${baseUrl}${s.wav}` : "";

        if (s.stem === "vocals") {
          vocalsUrl = mp3Url || wavUrl;
        }
        if (s.stem === "instrumental") {
          instrumentalUrl = mp3Url || wavUrl;
        }
      });

      if (!vocalsUrl || !instrumentalUrl) {
        throw new Error("Missing vocals or instrumental track");
      }
